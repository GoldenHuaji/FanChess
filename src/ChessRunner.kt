...
class ChessRunner(
    ...
    private val chessMapBackup: MutableList<HashMap<String, Any>> = mutableListOf()
    ...
    fun undo() {
        chessMap.forEachIndexed { index, chessItem ->
            chessMap[index] = null
        }
        chessMap = jsonToChessMap(chessMapBackup[chessMapBackup.lastIndex]["chessMap"]!! as Array<String>)
        turn = chessMapBackup[chessMapBackup.lastIndex]["turn"]!! as Int
        chessMapBackup.removeAt(chessMapBackup.lastIndex)
        doInit()
    }
    ...
    fun addToBackTree() {
        ...
        if (chessMapBackup.size != 0 && map == chessMapBackup.last()) {
            return
        }
        chessMapBackup.add(map)
    }
